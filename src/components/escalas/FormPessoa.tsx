...
// PERFIL (2026-09-27): nível e praças saíram daqui e foram pro Prontuário
// (avaliação da pessoa, só gestão); aqui fica só o que é escala.
...
import { REGIMES, SETORES, validarCadastro, type CadastroEscalaInput } from "@/lib/escalas/validacao";
...
export function FormPessoa({
  ...
  const previa = useMemo(() => {
    if (validarCadastro(c)) return null;
    const idPrevia = pessoa?.id ?? "__nova__";
    const editada = aplicarCadastro(idPrevia, c, pessoa?.perfil);
  ...
  }, [c, pessoa, pessoas, ocorrencias, regras, inicioPrevia, fimPrevia]);

  const salvar = async () => {
    const problema = validarCadastro(c);
    if (problema) return setErro(problema);
    setSalvando(true);
    const r = await aoSalvar(pessoa?.id ?? null, c);
    setSalvando(false);
  ...
  return (
    <Gaveta rotulo={pessoa ? `Escala de ${pessoa.nome}` : "Nova pessoa"} largura={560} aoFechar={aoFechar}>
      ...
      <div className="px-5 py-5 space-y-6 flex-1">
        <fieldset className="space-y-4">
          ...
          <label className="block">
            <Rotulo ajuda="Nível, praças e pontos fortes ficam no Prontuário da pessoa (aba Equipe).">Cargo</Rotulo>
