...
export function ChecklistsClient({ checklists, turnos }: { checklists: Checklist[]; turnos: Turno[] }) {
  ...
  return (
    <div className="max-w-5xl">
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11.5px]" style={{ color: C.faint }}>responsável</span>
          ...
          <button
            ...
            style={{ background: showNovoChecklist ? C.bg : C.text, color: showNovoChecklist ? C.text : "#fff", border: `1px solid ${showNovoChecklist ? C.borderStrong : C.text}` }}
          >
      ...
      {erroAcao && (
        <div className="text-[12.5px] mb-3 rounded-lg px-3.5 py-2 flex items-center justify-between" style={{ background: C.dangerSoft, color: C.danger }}>
          {erroAcao}
      ...
      {showNovoChecklist && (
        <Card className="mb-4">
          <div className="px-5 py-4">
            ...
            <button onClick={criarChecklist} className="text-[12.5px] font-medium px-3.5 py-1.5 rounded-lg" style={{ background: C.text, color: "#fff" }}>
              Criar checklist
            </button>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-2 gap-3">
            ...
            <Card key={ch.id} className="p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <div className="text-[10.5px] mt-0.5" style={{ color: C.faint }}>{MOMENTOS.find((m) => m.id === ch.momento)?.label}</div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div key={item.id} className="flex items-center gap-2 py-1">
                    {editando ? (
                      <>
                        ...
                        <button onClick={() => acaoRemoverItem(item.id).then((r) => !r.ok && setErroAcao(r.erro))} style={{ color: C.danger }}>
                          <X size={13} />
                        </button>
                      </>
                    ) : (
                      <button onClick={() => toggleItem(item.id, item.concluidoHoje)} className="flex items-center gap-2 text-left w-full">
                        <span className="w-3.5 h-3.5 rounded shrink-0 flex items-center justify-center" style={{ border: `1.5px solid ${item.concluidoHoje ? C.accent : C.borderStrong}`, background: item.concluidoHoje ? C.accent : "transparent" }}>
                          {item.concluidoHoje && <span style={{ color: "#fff", fontSize: 9, lineHeight: 1 }}>✓</span>}
                        </span>
                        <span className="text-[12px]" style={{ color: item.concluidoHoje ? C.faint : C.text, textDecoration: item.concluidoHoje ? "line-through" : "none" }}>{item.texto}</span>
                      </button>
                    )}
                  </div>
                ))}
              ...
              {editando && (
                <div className="flex gap-2 mt-3">
                  <input
                    ...
                    style={inputStyle}
                  />
                  <button onClick={() => addItem(ch.id, ch.itens.length)} className="text-[12px] font-medium px-3 py-1.5 rounded-md" style={{ background: C.text, color: "#fff" }}>
                    +
                  </button>
                  <button onClick={() => excluirChecklistComConfirmacao(ch)} className="text-[12px] font-medium px-3 py-1.5 rounded-md" style={{ color: C.danger, border: `1px solid ${C.borderStrong}` }}>
                    Excluir checklist
                  </button>
                </div>
              )}
        ...
        {checklists.length === 0 && !showNovoChecklist && (
          <div className="col-span-2 text-[12.5px] py-6 text-center" style={{ color: C.faint }}>
