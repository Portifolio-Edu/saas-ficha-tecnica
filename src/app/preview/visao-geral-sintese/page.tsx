...
/**
 * Colunas do kanban de produção. Cor por etapa (pedido explícito: "o kanban
 * tem que ter cor pra diferenciar as etapas") -- cada coluna carrega sua cor
 * no topo, no contador e na borda esquerda de cada card, não só um texto.
 */
const COLUNAS_KANBAN: { status: "em_producao" | "produzido" | "perda"; rotulo: string; cor: string }[] = [
  { status: "em_producao", rotulo: "Em produção", cor: "var(--g-amber)" },
  { status: "produzido", rotulo: "Produzido", cor: "var(--g-verde)" },
  { status: "perda", rotulo: "Perda", cor: "var(--g-vermelho)" },
];
...
export default function VisaoGeralSintese() {
    ...
    <div data-glass data-theme={tema} className={`g-fundo ${hanken.className}`}>
      <style>{`
        [data-glass][data-theme="light"] {
          ...
          --g-verde: #3C7A54; --g-azul: #3C6E9E; --g-vermelho: #B23F32; --g-amber: #B4791E;
        ...
        [data-glass][data-theme="dark"] {
          ...
          --g-verde: #5FAE7C; --g-azul: #6FA0D8; --g-vermelho: #E0685A; --g-amber: #E0AC5A;
        ...
        /* Kanban colorido: cada coluna carrega a cor da etapa na barra do
           topo, no contador e na borda esquerda de cada card -- não só um
           rótulo de texto. Pedido explícito: "o kanban tem que ter cor pra
           diferenciar as etapas". */
        .g-kanban { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; padding: 4px 24px 22px; }
        .g-kanban-col { border-radius: 14px; background: color-mix(in srgb, var(--kb-cor) 6%, var(--g-painel)); border: 1px solid color-mix(in srgb, var(--kb-cor) 22%, var(--g-painel-borda)); overflow: hidden; }
        ...
        <main className="g-main">
          ...
          <div className="g-painel" style={{ marginBottom: 16 }}>
            ...
            {resumo.perdasRecentes.length === 0 ? (
              <div style={{ padding: "0 24px 24px", color: "var(--g-faint)", fontSize: 12.5 }}>Nenhuma perda registrada ainda.</div>
            ) : (
          ...
          <div className="g-painel">
            ...
            <div className="g-secao-sub">Lotes em curso hoje, por etapa.</div>
            <div className="g-kanban">
              {COLUNAS_KANBAN.map((col) => {
                const itens = producoes.filter((p) => p.status === col.status);
                return (
                  <div key={col.status} className="g-kanban-col" style={{ ["--kb-cor" as string]: col.cor }}>
                    <div className="g-kanban-head">
                      <span className="g-kanban-rotulo">{col.rotulo}</span>
                      <span className="g-kanban-contagem" style={nums}>{itens.length}</span>
                    </div>
                    {itens.length === 0 && <div className="g-kanban-vazio">Nenhum lote nesta etapa.</div>}
                    {itens.map((it) => (
                      <div key={it.id} className="g-kanban-card">
                        <div className="g-kanban-card-nome">{it.nomeReceita}</div>
                        <div className="g-kanban-card-sub" style={nums}>{it.lote} · {it.quantidade} {it.unidadeRendimento}</div>
                      </div>
                    ))}
                  </div>
                );
